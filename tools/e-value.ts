...
export const LEAN_QUEUE: ReadonlyArray<{
  ...
  { cert: '(e-BH itself)', theorem: 'Tessera.EBH.fdr_le_nonneg', status: 'proved',
    validated: 'MACHINE-CHECKED (no sorry beneath it): count_antitone → kStar_mem/le_kStar → card_reject → fdp_pointwise → fdp_le_sum → fdr_le_of_pointwise → fdr_le_nonneg. Scope: NULL coordinates are e-values; non-null coordinates need only be nonnegative — so a detector behaving correctly under the alternative (E[e] > 1) still satisfies it. Cites fdr_le_nonneg rather than fdr_le since 2026-08-03: fdr_le demands IsEValue of EVERY coordinate, which is false exactly when a detector is working.' },
];
