...
export const LEAN_QUEUE: ReadonlyArray<{
  ...
  { cert: 'half-half-accumulator', theorem: 'Tessera.Conformal.accumulator_mean', status: 'proved',
    validated: 'MACHINE-CHECKED 2026-07-26 (with accumulator_ge_one, the NEGATIVE result: E[M_T] = E[g(D)^T] >= 1 — per-round validity does not survive accumulation). Statement REBUILT: the placeholder g was constant in the state, so the original statements were contentless; now on the honest conditional-i.i.d. kernel model. NOTE this row certifies the IDENTITY (and its adverse direction), NOT the supermartingale construction the cert claims — that premise remains a construction. Also MC vs shipped rank construction (test/exchangeability-drift.test.ts)' },
  ...
  { cert: '(e-BH itself)', theorem: 'Tessera.EBH.fdr_le_nulls_only', status: 'proved',
    validated: 'MACHINE-CHECKED (no sorry beneath it): count_antitone → kStar_mem/le_kStar → card_reject → fdp_pointwise → fdp_le_sum → fdr_le_of_pointwise → fdr_le_nulls_only. Scope: NULL coordinates are e-values and the FDP is integrable; NOTHING is assumed about the non-null coordinates — not an e-value, not integrability, not nonnegativity — nor about the joint law. So a detector behaving correctly under the alternative (E[e] > 1) satisfies it. Tightened twice on 2026-08-03: fdr_le demanded IsEValue of every coordinate, which is false exactly when a detector is working; fdr_le_nonneg then still demanded nonnegativity of every coordinate, which fdp_le_sum never used.' },
];
