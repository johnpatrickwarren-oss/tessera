...
export function mixWin(v: ReadonlyArray<number>, cS: number, cL: number, tS: number, tL: number): number {
  const cal = v.slice(cS, cS + cL); const { mean: mu, s2 } = moments(cal); const phi = estimateAr1(cal).phi;
  let S = 0, prevC = 0;
  for (let t = tS; t < tS + tL; t++) { const xc = v[t] - mu; S += xc - phi * prevC; prevC = xc; }
  return computeGaussianMixtureSupermartingale(S, tL, s2, s2);
}
...
export interface BFLifecycleReport {
  ...
  horizon_sweep: Array<{ n: number; mix_fp: number; bf_fp: number; mix_detect: number; bf_detect: number }>;
...
function horizonSweep(): BFLifecycleReport['horizon_sweep'] {
  ...
  return [30, 75, 150, 300, 600].map((n) => {
    let mixN = 0, bfN = 0, mixD = 0, bfD = 0;
    for (let s = 0; s < K; s++) {
      const vN = ar1(mulberry32(scramble(11 + s * 7)), CAL_LEN + n, -1, 0);
      if (mixWin(vN, 0, CAL_LEN, CAL_LEN, n) >= 1 / ALPHA) mixN++;
      if (bfWin(vN, 0, CAL_LEN, CAL_LEN, n) >= 1 / ALPHA) bfN++;
    ...
    return { n, mix_fp: round3(mixN / K), bf_fp: round3(bfN / K), mix_detect: round3(mixD / K), bf_detect: round3(bfD / K) };
  });
}
...
function renderMd(r: BFLifecycleReport): string {
  ...
  L.push('');
  L.push(`| n (test horizon) | n/m | mixSM FP | BF FP | mixSM detect | BF detect |`);
  L.push('|---|---|---|---|---|---|');
  for (const x of r.horizon_sweep) L.push(`| ${x.n} | ${round3(x.n / r.params.cal_len)} | ${pct(x.mix_fp)} | ${pct(x.bf_fp)} | ${pct(x.mix_detect)} | ${pct(x.bf_detect)} |`);
  L.push('');
  L.push('As the un-restarted horizon n grows past the calibration m, the **production mixSM over-fires** (invalid) while the **BF stays ≤ α** — both keep ~full detection. So the BF\'s advantage is REAL but confined to large-n (long accumulation without restart/re-record).');
  L.push('');
  if (r.real_blocks) {
    ...
    L.push('');
    L.push('With the lifecycle keeping n small (fresh calibration), the production mixSM is already valid — **BF ≈ mixSM (no improvement; BF slightly worse, its two-sample split spends data).** The remaining firing is REAL benign change, not estimation error (a lifecycle/fleet concern).');
    L.push('');
  }
  L.push('## Verdict');
  L.push('');
