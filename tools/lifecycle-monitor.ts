...
function renderMd(r: LifecycleReport): string {
  ...
  L.push('');
  L.push('**Reading it:**');
  L.push('- **Slow drift (FP):** static alarms pile up (baseline goes stale, ~154); the lifecycle re-records and cuts false alarms ~3× (toward adaptive) — *without* continuous adaptation.');
  L.push('- **Sharp fault:** all three detect 100% — a sharp fault fires on onset (the first alarm IS the detection), before any re-record matters.');
  L.push('- **SLOW fault (the needle):** static 70%, **adaptive masks (28%)** — but the **lifecycle keeps 70%**. A slow fault\'s *occasional* alarms don\'t trip the rate trigger (it barely re-records), so the lifecycle stays static-like and detects what adaptive adapts away. THIS is the value the lifecycle adds over continuous adaptation.');
  L.push('- **Continuous workload (FP):** the lifecycle DEGENERATES — a permanently high alarm rate makes it re-record constantly, collapsing toward adaptive (and would then mask). The honest limit: when within-epoch variability is *continuous* (not discrete drift), a single-shard lifecycle cannot separate legitimate change from faults — that needs the **fleet** (shard-specific vs fleet-wide).');
  L.push('');
  L.push('> **Verdict:** the lifecycle genuinely beats both static (far fewer drift false alarms) and adaptive (retains slow-fault detection adaptive masks) — for **discrete cross-epoch drift**, complementing m≫n (ADR 0009) and the seasonal 2D baseline (ADR 0010). It does NOT solve continuous within-epoch workload variability; that residual is irreducible at the single-shard level and needs fleet-relative comparison (which in turn needs the nuisance-robust e-value, ADR 0008). Shadow→cutover (validate a candidate baseline before promoting) is the safety wrapper around the re-record; modeled here as immediate cutover. The drift trigger is alarm-RATE (epoch-level), since the per-fire run-length does NOT separate drift from faults (verified).');
  L.push('');
  return L.join('\n');
}
