...
| P7 | **The operational form: a DRIFT condition, not a horizon.** `log M_t` is a random walk with drift `μ(δ) = E[log f(p)|δ]` (the Kelly log-growth rate); paging is first passage. Per-unit anytime page probability `≈ α^{κ(δ)}` where `κ(δ)` solves `E[f^κ|δ]=1` — **bounded by 1**, unlike `α·Λ`. Fleet rate `≈ N·P(δ ≥ δ₀)`, `δ₀(θ) = √(a₀² + (1+a₀²)θ²)`, `a₀ = 0.9128`. ⚠️ **The ICC target has been corrected TWICE — use neither 0.25% nor 9.5%.** Measured on the H15–H17 unit-persistence scenarios (2026-07-26): ICC 1.0% is safe to T=320; **ICC 9.2% runs 5× over the paging budget** and produces the first false e-BH selections seen anywhere. The steady-state form drops the sub-threshold `α^{κ(δ)}` bulk term. Verified-safe target is **ICC ≲ 1%**; the true boundary (1–9%) needs a sweep (A2-icc). Design against δ₀, which is stable; treat any rate as order-of-magnitude. | `research/2026-07-25-a2-tail-probability.md` (+ 2026-07-26 correction) |
