...
| ~~C-rack-channel~~ | **VERIFIED same day, after TWO integration gaps were found and closed** (research note § 3c addenda): (1) the nvlink per-host family on coarse keys leaked 1–6 false hosts — closed by host-cohort drafting + rack keys. (2) ⚠️ the group families were DEAD under rack-local blocking — group buffers ate within-rack ranks (rack-mean 0.5 by construction), so a 5 % rack fault detected d11.9 under coarse was invisible to EVERY family under rack-local. Closed by a coarse-keyed SECOND ranking pass feeding the group buffers (dedicated rng; coarse streams byte-identical). End state at 280 racks under heteroRackSd 1.0, rack-local: A/A 0 false selections/groups; rack fault d23.9 via rack family; unit faults 12 true / 0 false. Unit channel = rack-local ranks; group channel = coarse ranks with its studentized-change dispersion defenses. UNSWEPT: larger rack counts (N13-at-rack-count); group families stay EMP-CAL. |
| A2-disp-real | within-rack pair on real probe scores at the mini gate (~09-21) — the premise measurement |
| ~~C-rng-transfer~~ | **DISCHARGED 2026-07-29 by the tessera-rng session** (their ADRs 0059–0061, merged main): the onset-vs-N sweep ran on their fabric and the collapse DOES reproduce — onset ς̂ (0.31, 0.43] at 2016 leaves falls to ≤ 0.065 by 40k, with the tail-laundering channel confirmed at paper scale. Their per-leaf dispersion is NOT iid-protected. Their fix is construction-side rather than block-side: shrunk per-leaf scale calibration (their ADR-0052, measured N-robust to 6112 leaves) + an FDR license rule (0060: licensed ⇔ per_leaf_scale AND drift-monitor ok) + an N-indexed max-z triple gate (0061, the analog of our N13 cap). Same doctrine as ADR 0026 — construction-local validity + N-indexed gating — reached independently; no rack-local port needed on their side. |
