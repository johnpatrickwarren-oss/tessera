...
- **Mode B — FDR-guaranteed (CONDITIONAL, narrow).** e-BH-controlled discovery, admitted **only** for emitter contracts whose conditional null is `theorem_valid` or `construction_valid` over the horizon — in practice a **spatial null** (a concurrent control / canary, treatment − control), which cancels the common-mode the temporal null cannot. The guarantee is revocable: a live calibration monitor demotes an emitter B→A when its construction visibly breaks — with the caveat that any such monitor has violation classes it cannot see (measured: the pooled-marginal uniformity monitor is blind to persistent-heterogeneity drift, so the gated guarantee is `sup FDR ≤ max(q, β)` with β the monitor's miss rate against the violation class; [ADR 0023 Correction 2](./decisions/0023-active-canary-conformal-eprocess.md)). Since [ADR 0025](./decisions/0025-proof-carrying-e-values.md) the Mode B e-BH entry point is **proof-carrying**: `certifiedFdrBenjaminiHochberg` accepts only the opaque `EValue` type ([`tools/e-value.ts`](./tools/e-value.ts)), constructible solely via certified constructors that carry the validity argument — so a non-e-value (a p-value, an SR running max, a plug-in Bayes factor) entering the FDR path is a compile error, not a latent audit finding.
...
The always-on Mode B control loop ([`tools/mode-b-loop.ts`](./tools/mode-b-loop.ts)) wires FDR-controlled discoveries to the control plane via two deploy adapters: a live telemetry+control feed ([`tools/telemetry-source.ts`](./tools/telemetry-source.ts)) and pluggable [`ActionSink`s](./tools/action-sinks.ts) (rollout-gate / pager / remediation + durable audit). An active-probe sink ([`tools/diag-probe.ts`](./tools/diag-probe.ts)) closes the evidence→probe→verdict loop: a dispatch triggers cordon → escalating `dcgmi diag` → uncordon, and the verdict feeds back as an annotation (confirmed hardware stays cordoned; a *clean* diag reclassifies toward the SDC/workload hypothesis but never overrides the standing behavioral evidence). A per-topology-group probe quota (`groupOf`/`maxProbesPerGroup`) makes a common-mode fan-out probe a sample of the group and delegate the rest, so a rack-level discovery never evacuates the whole rack. The deployment-side wrapper ([`tools/dcgmi-diag.ts`](./tools/dcgmi-diag.ts)) supplies the shard→host mapping (hostmap file / template) and translates `dcgmi diag -j` JSON to the sink's exit-code contract — 0 pass · 1 fault · 2 orchestration error, so an unreachable host can never read as a confirmed hardware fault; its `exec` subcommand reuses the mapping for cordon/uncordon. See [`decisions/0019-two-mode-architecture-evidence-vs-fdr.md`](./decisions/0019-two-mode-architecture-evidence-vs-fdr.md).
