...
- **L0 contract for Tessera (counter-semantic preprocessing; Tessera-original; MR-1-amendment 2026-05-18).** SLICE 3.B precondition: define the explicit contract that Tessera's L0 ingestion layer guarantees to downstream consumers (TrendBuffer + per-shard detector cascade + fleet-merge). The contract bounds the work cleanly — once consumers can rely on stated L0 invariants, the inherited TrendBuffer and detector internals need no modification.

  **L0 contract (proposed invariants — exact wording lands at spec time):**
  1. **Rate-domain output, not counter-domain.** Every value pushed downstream is a rate (units per second) computed over the actual elapsed scrape interval. When `SchemaDescriptor.semantic_type === 'counter'` (per `engine/l0/schema-continuity.ts:44`), L0 computes `delta / actual_elapsed_seconds` per scrape-pair; raw cumulative counter values never reach downstream. Gauge / ratio / latency_quantile / categorical_rate types pass through value-domain unchanged.
  2. **Scrape interval is a first-class input, not assumed.** Each downstream value carries an `actual_elapsed_seconds` field computed per-pair from sample timestamps. The inherited `TrendBuffer` slope / `slopeNorm` / `roc` computations (`engine/core.ts:27-100`) assume uniform tick duration; the L0 contract addresses this by normalizing rates to per-second units BEFORE TrendBuffer ingestion. Comparable `slopeNorm` across configurable / variable scrape intervals follows from per-second-normalized inputs. (Alternative: TrendBuffer gains an elapsed-seconds-aware path; rejected as more invasive — modifying vendored engine internals violates A12. The L0-normalizes-first approach keeps the engine vendored-at-pin.)
  3. **Missed-scrape-then-catchup is the dangerous case; L0 detects and quarantines it.** The benign case (consistent scrape interval, clean deltas) is already handled correctly by raw-delta math. The failure mode is: scrape at t0, MISSED scrape at t0+Δ, catch-up scrape at t0+2Δ. Naive delta over the doubled interval produces an artificially-inflated rate — too large and too brief for `slowbleed`-class detectors to fire on, but well within the firing surface of `mfu_collapse` / `hbm_elevation` / similar rate-magnitude-sensitive detectors. L0 contract: when `actual_elapsed_seconds > expected_scrape_interval × (1 + jitter_tolerance)`, emit the rate with `slope_quality: degraded` AND a `missed_scrape_inferred: true` flag; downstream consumers MAY suppress detector firing on degraded samples per their own policy. Interpolation explicitly rejected (creates false structure that survives the degraded-flag).
  4. **Counter wraparound detection — DCGM 32-bit reality.** DCGM exposes both 64-bit and 32-bit counters; several NVLink error counters are 32-bit and can wrap on busy fabric over long runs (uint32 wraps at ~4.3B; an NVLink fabric pushing replay/CRC errors at fault-injection rates wraps in hours-to-days, not weeks). L0 contract: when `next < prev` AND `prev > (UINT32_MAX × 0.9)` (heuristic threshold; counter-width-aware via `SchemaDescriptor` metadata), classify as wraparound; emit rate via `(width_max - prev + next) / actual_elapsed_seconds`. When `next < prev` AND threshold not met, classify as counter reset (process restart); emit null + signal continuity-break to L0 schema-continuity layer per existing `breaking` classification (`engine/l0/schema-continuity.ts`).
  5. **L0 metadata propagation.** `slope_quality`, `missed_scrape_inferred`, `wraparound_handled`, `reset_detected` flags propagate alongside the rate value so downstream detector audits can attribute firings or non-firings to L0 preprocessing decisions.

  **Why "contract" framing.** Defining what L0 guarantees about its output (rather than enumerating preprocessing operations) bounds the SLICE 3.B work cleanly. The Architect spec'ies the contract surface; the Implementer's freedom is constrained to honoring the contract; the Reviewer audits both contract clarity and implementation conformance. Downstream consumers (TrendBuffer at `engine/core.ts`, per-shard detectors, fleet-merge consumer at `engine/fleet/verdict-consumer.ts`) build against the contract — they do not import L0 internals.

  **A10 carve-out granted** (see § 2.3 [MR-1 AMENDMENT] block above): measurement-domain L0 preprocessing is in-scope; hardware *diagnosis* (DCGM signal generation; per-GPU fault attribution; NVIDIA-stack tooling) remains fenced.
...
- **Cluster state history** — reuses inherited TrendBuffer / SignalSnapshot; no new ingestion required (but downstream of L0 counter-semantic preprocessing per above; the TrendBuffer remains value-domain — the preprocessing happens upstream).
...
- **A10: NO hardware-diagnostic territory.** DCGM / NVML integration, per-GPU hardware-fault attribution are NVIDIA-stack scope; Tessera consumes resulting signals as inputs (the inherited MFU / HBM / collective signals per `deploysignal/ARCHITECTURE.md` detectors at SHA `5a72371`), does NOT generate them. Per inherited TPM pre-route grilling discipline.
  ...
  >
  > A10 carve-out: **L0 contract for Tessera** is in-scope (see Extension 3 (b) L0 contract sub-extension below for the full proposed-invariants enumeration). The contract framing — defining what L0 guarantees to downstream consumers (rate-domain output; scrape interval as first-class input; missed-scrape-then-catchup detection; DCGM 32-bit wraparound handling; metadata propagation) — bounds the work cleanly without modifying inherited engine internals (A12 preserved). The benign case (consistent scrape interval, clean deltas) is correctly handled by raw-delta math today; the dangerous failure mode addressed is the missed-scrape-then-catchup spike (artificially-inflated rate over doubled interval — too brief for slowbleed but well within mfu_collapse / hbm_elevation firing surface). Lands as a Phase 2 SLICE 3 sub-row precondition (§ 3 Q-cycle row "Phase 2 SLICE 3.A.5 — L0 contract"). Failure mode addressed: scrape-artifact-fires-as-signal (R-E7; § 4.2). Operator-raised 2026-05-18 morning with three sharpening details (missed-scrape-catchup dynamic; scrape interval as first-class input; DCGM 32-bit wrap); operator-authorized amendment same session.
...
| Phase 2 SLICE 3.A | 1 | HardwareTopologySource scaffold (TopologySource concrete impl class; v9Y multi-rack fixture; TopologyNode.kind + TopologyEdge.relationship enum extensions for PSU / cooling-zone / nvlink_peer). Landed at R23 ✅. |
| Phase 2 SLICE 3.A.5 | 1-2 | **L0 contract for Tessera (MR-1 amendment 2026-05-18).** SLICE 3.B precondition. Defines what Tessera's L0 ingestion layer guarantees to downstream consumers: rate-domain output (counter→rate at L0; raw counters never reach TrendBuffer); scrape interval as first-class input (per-sample `actual_elapsed_seconds`; comparable slopeNorm across configurable / variable intervals); missed-scrape-then-catchup detection (`slope_quality: degraded` + `missed_scrape_inferred: true`; addresses the artificially-inflated-rate failure mode for mfu_collapse / hbm_elevation); DCGM 32-bit counter wraparound handling (heuristic `next < prev` + `prev > UINT32_MAX × 0.9`); reset-vs-wrap disambiguation; L0 metadata propagation for audit attribution. Inherited engine internals unmodified (A12 preserved); contract framing bounds work cleanly. See § 2.3 Extension 3 (b) L0 contract sub-extension for full invariant enumeration. Addresses R-E7. |
| Phase 2 SLICE 3.B | 2 | HardwareTopologySource concrete ingestion adapters (Slurm topology format / Kubernetes node-label API / NVIDIA NVLink-topology output → TopologySnapshot). Consumes L0 contract (SLICE 3.A.5) by interface — adapters know they receive rate-domain values with elapsed-seconds metadata; do not re-implement counter handling. |
| Phase 2 SLICE 3.C | 1 | Topology-aware spatial attribution (MD-F4; PR-F6 pair-review including BFS-on-undirected evaluation). Common-mode failure-injection empirical test (rack-localized PSU event simulation on synthetic v9Y cluster substrate). **Hybrid Reviewer pair-review-style at SLICE 3 close.** |
...
| **R-E7 (NEW MR-1 2026-05-18)** — Counter-as-rate scrape artifact. Pushing raw cumulative counter values into the inherited `TrendBuffer` (`engine/core.ts:27-100`) produces monotonic positive `slopeNorm` that is the long-run mean rate, not actual rate dynamics. **Dangerous failure mode is missed-scrape-then-catchup**: scrape at t0, missed scrape at t0+Δ, catch-up at t0+2Δ → naive delta over doubled interval produces an artificially-inflated rate, too brief for `slowbleed` to fire on but well within `mfu_collapse` / `hbm_elevation` firing surface. Additional failure: DCGM 32-bit NVLink error counters can wrap on busy fabric (hours-to-days at fault-injection rates) and look like catastrophic spikes. Additional failure: variable-scrape-interval breaks direct `slopeNorm` comparability since TrendBuffer assumes uniform tick duration. | A10 clarification ([MR-1 AMENDMENT] § 2.3) carves out L0 preprocessing as in-scope. SLICE 3.A.5 "L0 contract for Tessera" (§ 3 Q-cycle row above) defines guaranteed invariants downstream consumers can rely on. | Architect spec for SLICE 3.A.5 enumerates exact L0 contract surface (rate-domain output; per-sample `actual_elapsed_seconds`; missed-scrape detection + `slope_quality: degraded`; DCGM 32-bit wrap heuristic; reset-vs-wrap disambiguation). Empirical validation deferred to SLICE 3.B adapters (each adapter's tests must exercise the missed-scrape, wrap, and variable-interval cases against a synthetic counter generator). |
