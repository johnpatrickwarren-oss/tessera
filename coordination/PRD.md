...
- WAVE-GATE-06 closure: aggregate ALLOWED_SET union check via `scripts/verify-wave-aggregate.sh WAVE-06`; tier-aware consolidation Reviewer per R50 design (all sub-WUs full-tier; consolidation Reviewer OPTIONAL). (Naming: WAVE numbering is globally sequential across phases per R50 verifier convention. Phase 2 used WAVE-GATE-01 through 05 — so Phase 3's first wave = WAVE-06; WAVE-PLAN-06.md is the matching plan filename. Asymmetry between WAVE-PLAN-NN count (06) and WAVE-GATE-NN count (06+) is structural — Phase 2 had 3 plans dispatching 5 waves; Phase 3 SLICE 1 has 1 plan dispatching 1 wave.)
