...
| AC-R26-14 | Given the chore-A SHA `<CHORE-A-SHA>`, when `npx tsc -p tsconfig.test.json` is run from the worktree root, then the exit code is 0 (zero diagnostics). | Typecheck binding-command (per R22 IMPL MINOR-1 + R23 chore-A-SHA-anchoring); attestation in NEXT-ROLE.md. | § 3.1 typecheck must succeed. |
