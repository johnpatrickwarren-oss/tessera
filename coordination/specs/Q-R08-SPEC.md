...
3. **In-passing carry-forward MINOR closures** (operator-listed at NEXT-ROLE.md "In-passing items R08 MAY close" — Architect's discretion; included because each is a single-character / 1-2-line edit, well-bounded, low risk):
   ...
   - **R07 MINOR-3**: AC-15 `<= origLen` length assertion — initially R08 prescribed tightening to `=== origLen` based on the (empirically incorrect) premise that MCD drops zero ticks on the clean alternating-pattern fixture. R09 corrected the premise (see § Mechanism primitive 11 R09 EMPIRICAL CORRECTION addendum at line 74): MCD actually drops 2 ticks per run × 3 runs = 6 total. Disposition: keep `<= origLen` (preserves the bounded-by-original-length invariant) AND add a sibling binding `n_ticks_contaminated === 6` to lock the MCD-drop count explicitly. Applied at R09.
...
11. **D-R08-11 — Closes R07 MINOR-3: AC-15 weak length assertion.** ⚠️ **R09 EMPIRICAL CORRECTION (2026-05-16)**: The load-bearing premise in this primitive is empirically FALSE. R09 empirical probe (`curateBaselineFleetCorrelated` on the clean-fleet-v1 bundle): `n_ticks_contaminated=6` (2 ticks × 3 runs; origLen=8/run, curatedLen=6/run) — MCD flags 2 ticks per run on the clean alternating-pattern fixture. The original premise ("zero Stage 2a contamination flags") was inherited from R07 Reviewer MINOR-3 testimony without independent verification (R08 MAJOR-2 root cause). The prescribed tightening to `assert.strictEqual(curatedLen, origLen, …)` was correctly reverted by the Implementer when it failed empirically (R08 MAJOR-1 discipline finding; the missing piece was the required DIAGNOSTIC + STATUS: ESCALATE). R09 closes this via approach (γ) per Q-R09-SPEC.md §2 Decision A: adds `assert.strictEqual(result.decisions.D11!.output_summary.n_ticks_contaminated, 6)` to the AC-15 test body (documents MCD's empirical Stage-2a behavior; origLen=8/run, curatedLen=6/run) and preserves the `<= origLen` loop (Stage-2b-no-drop evidence). The `<= origLen` assertion remains correct: curated lengths (6) are ≤ original lengths (8). — Original R08 spec text (the prescribed tightening was not applied; see MAJOR-1 in REVIEWER-REPORT-R08.md): Line 339 of the q07 test file currently asserts `assert.ok(curatedLen <= origLen, …)` on a clean-fleet fixture. Single-line edit. The clean fixture at lines 322-330 has been verified by the R07 Reviewer to produce `fcp1State.fired === false` (no Stage 2b drop). [Premise about Stage 2a contamination count was wrong — corrected above.]
...
| 11 | R07 MINOR-3 disposition (R09 AMENDED): keep AC-15 `<= origLen`; add sibling `n_ticks_contaminated === 6` binding — per D-R08-11 amended by R09 empirical investigation | § Mechanism primitive 11 + § Per-file pseudocode Delta 11 (both carry R09 EMPIRICAL CORRECTION addendum) | original `=== origLen` (failed empirically: curatedLen=6, origLen=8 — MCD drops 2 per run); revert without sibling binding (loses MCD-drop-count documentation) | Pseudocode at Q-R08-SPEC.md as R09-amended; test at test/q07-fleet-correlated.test.ts:363-372 |
...
All 18 checks PASS at spec-emit time. (R09 amendment: row 11 was based on an empirically-wrong premise about MCD behavior; R09 amended the disposition. The cross-section table is consistent post-R09-amendment — the AC-15 binding documented in row 11 + the test file + § Mechanism primitive 11 R09 addendum + § Per-file pseudocode Delta 11 R09 addendum all agree on the keep-`<= origLen` + add-`n_ticks_contaminated === 6`-sibling disposition.)
...
The clean-fleet fixture (current lines 322-330) has measured Stage 2a contamination of 2 ticks per run × 3 runs = 6 total ticks (R09 empirical correction; R08 originally claimed zero based on inherited testimony — see § Mechanism primitive 11 R09 EMPIRICAL CORRECTION addendum at line 74). No Stage 2b fire (fcp1State.fired===false verified at line 332). Post-curation length is bounded by original length (`<= origLen`) but not equal to it; the per-run shortfall equals the MCD drop count. The AC-15 binding (post-R09) asserts `<= origLen` AND `n_ticks_contaminated === 6` as sibling claims.
...
- **AC-15 (modified at R08; R09 AMENDED post-empirical-investigation)** — _Given_ a clean fleet BaselineBundle (3 runs × 8 ticks × 2 signals, no outliers), _when_ `curateBaselineFleetCorrelated(bundle)` is called, _then_ `result.fcp1State.fired === false` AND `result.decisions.D12!.output_summary.fired === false` AND `result.decisions.D12!.output_summary.fire_window === -1` AND `result.decisions.D11!.output_summary.n_ticks_contaminated === 6` (R09 sibling binding: MCD drops 2 ticks per run × 3 runs) AND for each run, `result.curatedBundle.runs[k].signal_series.a.length <= bundle.runs[k].signal_series.a.length` (length bounded by original; per-run shortfall = MCD drop count). R09 amendment: R08 originally prescribed `=== origLen` exact-equality based on the wrong premise that MCD drops zero ticks; empirical investigation showed MCD drops 2 per run. The `<= origLen` bound + the explicit `n_ticks_contaminated === 6` sibling binding together document the correct behavior. Evidence: `test/q07-fleet-correlated.test.ts` "R07 AC-15 …" passes (test name preserved; assertion amended per R09 Delta 11 EMPIRICAL CORRECTION).
