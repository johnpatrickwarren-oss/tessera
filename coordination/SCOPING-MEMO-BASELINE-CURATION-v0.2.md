# SCOPING-MEMO — Tessera Baseline Curation v0.2

_Author: Tessera architect (operator-led scoping; assisted draft; v0.2 amends v0.1 § Q-JC4 + adds Q-JC4a/b/c per math-framing correction)._ Companion to `SCOPING-MEMO-v0.3.md`. Q-cycle path candidate for Tessera Phase 1 SLICE 4 (or later). Awaits John's Q-JC dispositions before any Architect-role round drafts a formal spec._
...
_Memo v0.2 authored: 2026-05-16. Amends v0.1 § Q-JC4 (statistical formulation corrected from K%-threshold to sequential e-process); adds Q-JC4a/b/c (mixture/prior choice; p_base estimation strategy with conditional-independence constraint; Q-J1 e-BH coupling). Stage 2b in § 2 and R-C1 in § 4 updated to match. Awaits John's Q-JC dispositions before Architect SLICE 4 spec emit. Memorial D state at memo-emit time: 22V/8C (no increment in v0.1 or v0.2; v0.1 file-opened-discipline applied via direct read of `tools/curate-baseline-pipeline.ts`; v0.2 math correction caught at operator-review time before any architect round consumed v0.1's flawed framing — the cost of the correction was a 10-minute memo revision rather than a multi-round SLICE 5 fix cycle). Skill 14 + 15 pre-route gates: scope-proposal fidelity at this memo; SLICE 4 spec will apply gates at SPEC fidelity per architect role. PR-F8 pair-review trigger now explicitly requires formal verification of the martingale property under the chosen `p_base` estimation strategy._
