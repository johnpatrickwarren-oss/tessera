# CLUSTER-HANDOFF-1-WU00-WU01 — WU-00 L0-CONTRACT → WU-01 SLURM-ADAPTER

**From:** Coordinator TPM (R27)
**Date:** 2026-05-18
**Wave:** Source cluster CL-01-A (Wave 1) → Target cluster CL-02-A (Wave 2)
**Foundation:** `WAVE-PLAN-02.md` §WU-00 + §WU-01; `coordination/reviews/REVIEWER-REPORT-R25.md`; `coordination/WAVE-GATE-01.md`
...
WU-01 (Slurm topology-format adapter) consumes the L0 contract surface that WU-00 landed. This artifact is the interface WU-01's Architect builds against — not a pointer to go read WU-00's spec, but the concrete description of the function signature, types, and behavioral guarantees the SLURM adapter receives.
...
- **Target cluster:** CL-02-A
- **Target work unit:** WU-01 — SLURM-ADAPTER
- **Dependency test that fired:** D2 (AC reference / interface contract)
- **Edge confidence:** MEDIUM
- **Edge reasoning:** Per SCOPING-MEMO § 3 SLICE 3.B line 365 ("adapters know they receive rate-domain values with elapsed-seconds metadata; do not re-implement counter handling"), the SLURM adapter consumes the L0 contract surface BY INTERFACE. Slurm `topology.conf` parses cluster topology configuration — not counter-typed surfaces — so the dependency is interface-only (not direct counter-arm exercise like WU-03). The L0 contract still binds because the Slurm adapter is expected to construct `CounterMetadata` for any signals it forwards and pass them through `transformPair` rather than re-implementing per-adapter rate semantics.
...
- WU-00 did NOT produce a `TopologySource` interface implementation — that is WU-01's scope.
- WU-00 did NOT produce any Slurm-specific parsing code — that is WU-01's scope.
...
- `transformPair`'s output is NOT guaranteed to handle multi-counter aggregation. If the Slurm adapter produces multiple counters per scrape pair, it invokes `transformPair` once per counter; aggregation across counters (if any) is the adapter's responsibility.
...
- **R25 MINOR-3 advisory — gauge + missed_scrape combination not behaviorally bound.** If WU-01's adapter tests push any gauge value through `transformPair` with a missed-scrape-shaped interval (elapsed > expected × 1.5), they can incidentally close the R25 MINOR-3 coverage gap. Not load-bearing for WU-01 acceptance; optional.
...
2. The Slurm `topology.conf` parsing surface requires a contract type not present in WU-00's exports (`CounterSample` / `CounterMetadata` / `TransformOpts` / `RateSample` insufficient) → route back; the handoff artifact needs amendment, not a cluster-internal workaround.
...
|---|---|---|---|
| v1 | 2026-05-18 | Initial creation | Wave 1 gate (R27) — authorizing Wave 2 dispatch of WU-01 SLURM-ADAPTER |
