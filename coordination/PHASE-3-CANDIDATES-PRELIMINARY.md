...
- **Anchor canonical landing path (3+ occurrence threshold; deferred):** This problem is universal to anchor, not Tessera-specific — any sufficiently-long anchor project hits monotonic memorial growth. **BUT** anchor canonical-landing should follow the Rule 7 (`derived-rule-propagation-mechanism-required`) discipline this session derived: validate the solution empirically in Tessera first (MR-3); wait for a 2nd anchor-using project to surface the same scaling problem; then canonicalize as methodology recognition + decision tree of strategies (a-e) + reference implementation pointer — NOT a specific mechanism mandated for all projects. Captured in `coordination/ANCHOR-BACKFLOW-2026-05-18.md` § 7 with the full backflow-candidate framing.
- **Why defer to 2nd-project occurrence:** Memorial-sharding canonical-landing without empirical proof would be exactly the pattern Rule 7 warns against. Tessera is 1 data point. The same Tessera-internal benefit holds (MR-3 lands; per-round read cost drops); only the cross-project canonicalization is gated on the 3+ threshold.
