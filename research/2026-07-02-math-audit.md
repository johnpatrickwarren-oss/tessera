...
marked ⚠️ STANDING were open work **as of 2026-07-02**.

> **STATUS HEADER (added 2026-07-31) — read before the markers below.**
>
> The finding text is a dated record and is left as written; the ⚠️ STANDING markers are not
> current. **Both standing findings are now closed:**
>
> - **F1** (engine nuisance-robust BF is not an e-value) — closed **2026-07-02**, same day as this
>   audit, by engine **v0.6.2-pre**: `nuisanceRobustBFEValue` `@deprecated`, envelope flipped to
>   `validUnderEstimatedBaseline: false` (so `isValidForFdrPath`/`assertValidForFdrPath` no longer
>   auto-admit it), all pointers routed to safe-t, regression test demonstrating E[BF|H0] > 1 in an
>   MC-sampleable regime. See `../deploysignal-engine/CHANGELOG.md` § v0.6.2-pre.
> - **F6** (UI "any φ by construction" proof hole) — closed **2026-07-29** by engine **ADR 0025**,
>   the sequential-UI e-process (`detectors/sequential-ui.ts`, PR #41): predictable-plug-in
>   conditional likelihood over a profiled null sup, E[E_τ] ≤ 1 at every stopping time for any φ.
>   This is opportunity 6 in "Free power on the table" below, built. See
>   `research/2026-07-29-audit-stragglers-closed.md` § 1.
>
> That document also closes the other three research-scale stragglers: **O2** (2026-07-28,
> robust-Catoni fit measured → engine ADR 0027), **ADR-0015-v2** (2026-07-29, superseded by
> measurement — the gap is coverage economics, not a missing construction), and **shadow-replay**
> (2026-07-29, regenerated byte-identical under the post-audit + ADR 0025/0026/0027 tree). F6's
> closure also profiles out **F7**'s σ̂-sensitivity for the sequential-UI case. The 🔧 PARTIAL
> markers (F7, F9, F11) otherwise still describe their state.
...
## F1 — Engine nuisance-robust BF is not an e-value (CRITICAL; ⚠️ STANDING at audit time — CLOSED 2026-07-02, engine v0.6.2-pre; see status header)
...
## F6 — UI "any φ by construction" proof hole (GAP; documented in RESEARCH-INDEX P1; ⚠️ STANDING at audit time — CLOSED 2026-07-29, engine ADR 0025; see status header)
